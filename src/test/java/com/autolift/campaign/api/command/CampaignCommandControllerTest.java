...
class CampaignCommandControllerTest {
  ...
  @MockBean private CreateCampaignCommandHandler createHandler;

  @MockBean private ActivateCampaignCommandHandler activateHandler;
  ...
  void shouldCreateCampaign() throws Exception {
    ...
    CampaignCreatedResult result =
        new CampaignCreatedResult(
  ...
  void shouldActivateCampaign() throws Exception {
    ...
    CampaignActivatedEvent event =
        new CampaignActivatedEvent(campaignId, "Summer Sale", Instant.now());
  ...
  void shouldPauseCampaign() throws Exception {
    ...
    mvc.perform(post("/api/campaigns/{id}/pause", campaignId)).andExpect(status().isNoContent());
  ...
  void shouldCompleteCampaign() throws Exception {
    ...
    mvc.perform(post("/api/campaigns/{id}/complete", campaignId)).andExpect(status().isNoContent());
