...
public class CampaignQueryController {
    ...
    public CampaignQueryController(
            GetCampaignQueryHandler getHandler,
            GetAllCampaignsQueryHandler getAllHandler) {
    ...
    public List<CampaignResponse> findAll() {
        return getAllHandler.handle(new GetAllCampaignsQuery()).stream()
            .map(this::toResponse)
            .toList();
    }
    ...
    public ResponseEntity<CampaignResponse> findById(@PathVariable String id) {
        return getHandler.handle(new GetCampaignQuery(id))
