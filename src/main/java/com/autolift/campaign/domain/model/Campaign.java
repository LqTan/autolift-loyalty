...
public class Campaign {
  ...
  public static Campaign create(
      String name, String description, Instant startDate, Instant endDate, Budget budget) {
    CampaignId id = CampaignId.random();
    Instant now = Instant.now();
    return new Campaign(
        id, name, description, CampaignStatus.DRAFT, startDate, endDate, budget, now, now);
  }

  public static Campaign of(
      CampaignId id,
      String name,
      String description,
      CampaignStatus status,
      ...
      Instant updatedAt) {
    return new Campaign(
        id, name, description, status, startDate, endDate, budget, createdAt, updatedAt);
  }
  ...
  public void pause() {
    if (this.status != CampaignStatus.ACTIVE) {
      throw new InvalidCampaignStateException("Cannot pause campaign with status: " + this.status);
    }
