...
public class CreateCustomerCommandHandler {
  ...
  public CreateCustomerCommandHandler(
      CustomerRepository repository, ApplicationEventPublisher eventPublisher) {
  ...
  public CreateCustomerResult handle(CreateCustomerCommand command) {
    Customer customer =
        Customer.create(command.name(), command.email(), command.phone(), command.segment());
    repository.save(customer);
    eventPublisher.publishEvent(
        new CustomerCreatedEvent(
            customer.getId().getId().toString(), customer.getName(), customer.getEmail()));
